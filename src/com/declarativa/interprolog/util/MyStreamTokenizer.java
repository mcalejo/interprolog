...
Author of changes: Miguel Calejo
Contact: info@interprolog.com, www.interprolog.com
Copyright InterProlog Consulting / Renting Point Lda, Portugal 2014
Use and distribution of this version, without any warranties, under the terms of the
...
public class MyStreamTokenizer {
	...
	if ((ctype & CT_DIGIT) != 0) {
	    ...
	    // This was being created everytime:
	    sb.delete(0,sb.length());
	    /*int now an instance variable:*/ seendot = 0;
		...
		if (seendot==1)
			v=Double.valueOf(number);
		else v = Long.parseLong(number);
		number = null;
	    nval = neg ? -v : v;
	...
	if ((ctype & CT_QUOTE) != 0) {
	     ...
	     */
	    int d = read();
	    while (d >= 0 && d != ttype /* allow multiline quoted strings  && d != '\n' && d != '\r' */) {
	        /* We do NOT want to assign meaning to escape sequences emanating from the Prolog side:
	        if (d == '\\') {
   		    c = read();
    ...
    public void dump(int nChars){
    	try {
			if (reader!=null){
				int i = 0;
				int C;
				while ((C = reader.read())!=-1 && i++ <nChars){
					System.err.print((char)C);
				}
				System.err.println();
			}
			if (input!=null){
				throw new IPException("weird tokenizer");
			}
    	} catch (Exception e){
    	}
    }
